...
def events(request: Request, user: str):
    ...
    # OPTIMIZATION: Removed the waste loop (range 3000000) that was blocking execution.
...
def my_events(request: Request, user: str):
    db = get_db()
    # The SQL query here is already efficient, so we keep it.
    rows = db.execute(
        """
    ...
    # OPTIMIZATION: Removed the dummy loop (range 1500000) that was slowing down the response.
